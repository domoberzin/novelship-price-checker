...
def search_novel(search_query):
    ...
    search_url = "https://novelship.com/api/products/search?where[search]=" + 
    search_query + "&where[active:eq]=true&page[number]=0&page[size]=20"
    ...
    if search["results"] != []:
        ...
        a = requests.get("https://novelship.com/api/products/slug/"+ 
                         name_slug + "?where[active:eq]=true",headers=headers_dict).json()
        print(a.keys())
        ...
        found_url =  "https://novelship.com/api/products/" + pid +  
                    "/offer-lists?where[active:eq]=true&modifier[c]=3&page[number]=0&page[size]=1000"
        ...
        for i in range(len(found["results"])):
            try:
                if found["results"][i]["type"] == "buying":
                    ...
                    buying_price_list.append(((str(found["results"][i]["size"])),int(f"{price:.0f}"), 
                                              str((f"{(price*0.884- 6.80 - 0.70):.0f}")),(float(found["results"][i]["size"][3:]))))
                if found["results"][i]["type"] == "selling":
                    price2= found["results"][i]["price"]
                    selling_price_list.append(((str(found["results"][i]["size"])),int(f"{price2:.0f}"), 
                                               str((f"{(price2*0.884- 6.80 - 0.70):.0f}")),(float(found["results"][i]["size"][3:]))))
                ...
                if found["results"][i]["type"] == "buying":
                    price = found["results"][i]["price"]
                    buying_price_list.append(((str(found["results"][i]["size"])),int(f"{price:.0f}"), 
                                              str((f"{(price*0.884- 6.80 - 0.70):.0f}"))))
                if found["results"][i]["type"] == "selling":
                    price2= found["results"][i]["price"]
                    selling_price_list.append(((str(found["results"][i]["size"])),int(f"{price2:.0f}"), 
                                               str((f"{(price2*0.884- 6.80 - 0.70):.0f}"))))
